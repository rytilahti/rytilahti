...
def __():
    ...
    for _package in PYPI_PACKAGES:
        try:
            pypi_recent[_package] = json.loads(pypistats.recent(_package, format="json"))["data"]["last_month"]
        except Exception as e:  # catch broadly to handle rate limits, timeouts, or service outages
            print(f"Warning: could not fetch pypistats for {_package}: {e}")
